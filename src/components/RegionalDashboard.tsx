  ...
  return (
    <div className="space-y-6">
      ...
      <DonutChart
        ...
        items={[
          { name: 'Mão de Obra', value: costData.find(c => c.name === 'Mão de Obra')?.value ?? 0, color: '#378ADD' },
          { name: 'Matéria Prima', value: costData.find(c => c.name === 'Matéria Prima')?.value ?? 0, color: '#1D9E75' },
          { name: 'Impostos', value: costData.find(c => c.name === 'Impostos')?.value ?? 0, color: '#EF9F27' },
        ]}
      />
