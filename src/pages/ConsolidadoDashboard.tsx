...
export default function ConsolidadoDashboard() {
  ...
  const alerts = generateAlerts(filtered, sysConfig);
        ...
        >
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-muted-foreground/60">Base: {projecao.baseMonths.join(", ")}</span>
            </div>
            <div
              className={`flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${
                projecao.tendencia > 0.5
          ...
          <p className="text-[10px] text-muted-foreground/60 mt-3 text-right">
            Projeção estatística baseada na média dos últimos {projecao.baseMonths.length} meses. Não substitui análise
            gerencial.
          </p>
        </motion.div>
      )}
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-3 text-[10px] font-semibold flex-wrap">
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded-sm bg-success inline-block" /> ≥ Meta
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded-sm bg-success/50 inline-block" /> 80-100% da meta
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded-sm bg-warning/70 inline-block" /> 50-80% da meta
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded-sm bg-danger/50 inline-block" /> &lt;50% da meta
            </span>
            <span className="flex items-center gap-1">
                          ...
                          >
                            ...
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 hidden group-hover:flex flex-col items-center z-10 pointer-events-none">
                              <div className="bg-background border border-border rounded-lg p-2 shadow-lg text-left whitespace-nowrap">
                                ...
                                {u.meta > 0 && (
                                  <p className="text-[9px] text-muted-foreground">Meta: {u.meta.toFixed(1)}%</p>
                                )}
                                {u.perfPct !== null && (
                                  <p
                                    className={`text-[9px] font-semibold ${u.perfPct >= 100 ? "text-success" : u.perfPct >= 80 ? "text-warning" : "text-danger"}`}
                                  >
                                    {u.perfPct.toFixed(0)}% da meta
                                  </p>
                                )}
                              </div>
