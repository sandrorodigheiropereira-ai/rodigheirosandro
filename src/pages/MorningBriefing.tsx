        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-2">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <span
                className={`text-xs font-medium ${reg.margem < 0 ? "text-danger" : reg.margem < 5 ? "text-warning" : "text-success"}`}
              >
                ...
                return (
                  <tr key={u.unidade} className="border-t border-border/50 hover:bg-secondary/30 transition-colors">
                    ...
                    <td className="p-2 text-right text-muted-foreground">
                      {formatCurrency(
                        lastMonthRecords
                          .filter((r: any) => r.unidade === u.unidade)
                          .reduce((s: number, r: any) => s + r.receitaBruta, 0),
                      )}
                    </td>
                    <td
                      className={`p-2 text-right font-medium ${u.metrics.margem < 0 ? "text-danger" : u.metrics.margem < 5 ? "text-warning" : "text-success"}`}
                    >
                      {formatPercent(u.metrics.margem)}
                    </td>
                    <td
                      className={`p-2 pr-4 text-right font-medium ${u.metrics.cmvPercent > 50 ? "text-danger" : u.metrics.cmvPercent > 40 ? "text-warning" : "text-success"}`}
                    >
...
function RegionalRankingList({
  rankingRegional,
  lastMonthRecords,
  formatCurrency,
  formatPercent,
}: {
  rankingRegional: RegionalItem[];
  lastMonthRecords: any[];
  formatCurrency: (v: number) => string;
  formatPercent: (v: number) => string;
}) {
...
export default function MorningBriefing() {
  ...
  const months = useMemo(() => [...new Set(allRecords.map((r) => r.data))].filter(Boolean).sort(), [allRecords]);
  const lastMonth = months[months.length - 1];
  const prevMonth = months.length >= 2 ? months[months.length - 2] : null;

  const lastMonthRecords = useMemo(() => allRecords.filter((r) => r.data === lastMonth), [allRecords, lastMonth]);
  ...
  const alerts = useMemo(() => generateAlerts(lastMonthRecords, sysConfig), [lastMonthRecords]);
  ...
  const rankingRegional = useMemo(() => {
    return regionais
      .map((reg) => {
        ...
        const avgScore =
          unidadesReg.length > 0 ? Math.round(unidadesReg.reduce((s, u) => s + u.score, 0) / unidadesReg.length) : 0;
        return {
          regional: reg,
  ...
  const topMargem = useMemo(() => rankUnidades(lastMonthRecords, "margem").slice(0, 3), [lastMonthRecords]);
  ...
  const pct = (curr: number, prev?: number) => (prev && prev > 0 ? ((curr - prev) / prev) * 100 : undefined);
  ...
  const semaforoConfig = {
    ...
    vermelho: {
      label: `${dangerAlerts.length} alerta${dangerAlerts.length > 1 ? "s" : ""} crítico${dangerAlerts.length > 1 ? "s" : ""}`,
      color: "text-danger",
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          },
          {
            ...
            suffix: prevMetrics
              ? `${metrics.cmvPercent - prevMetrics.cmvPercent > 0 ? "+" : ""}${(metrics.cmvPercent - prevMetrics.cmvPercent).toFixed(1)}pp vs ${prevMonth}`
              : undefined,
          ...
          >
            ...
            {kpi.suffix && (
              <div
                className={`flex items-center gap-1 mt-2 text-[11px] font-medium ${kpi.positive === true ? "text-success" : kpi.positive === false ? "text-danger" : "text-muted-foreground"}`}
              >
                {kpi.positive === true ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div
          ...
          className="glass-card rounded-xl p-5 space-y-3"
        >
          <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Saúde por regional</h3>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-0.5">
                      <span className="text-[10px] text-muted-foreground">
                        Margem:{" "}
                        <span className={r.margem < 0 ? "text-danger" : r.margem < 5 ? "text-warning" : "text-success"}>
      ...
      >
        ...
        <RegionalRankingList
          rankingRegional={rankingRegional}
          lastMonthRecords={lastMonthRecords}
          formatCurrency={formatCurrency}
          formatPercent={formatPercent}
        />
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "% MdO Médio",
              value: `${rhLastMonth.avgPct.toFixed(1)}%`,
              color:
                rhLastMonth.metaAvg > 0 && rhLastMonth.avgPct > rhLastMonth.metaAvg ? "text-danger" : "text-success",
            },
            {
              label: "Funcionários",
              value: rhLastMonth.totalFunc > 0 ? rhLastMonth.totalFunc.toFixed(0) : "—",
              color: "text-foreground",
            },
            {
              label: "Hora Extra",
              value: rhLastMonth.totalHe > 0 ? formatCurrency(rhLastMonth.totalHe) : "—",
              color: rhLastMonth.totalHe > 0 ? "text-warning" : "text-muted-foreground",
            },
          ].map((kpi) => (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="w-full h-1 rounded-full bg-secondary">
                    <div
                      ...
                      style={{ width: `${(u.horaExtra / rhLastMonth.topHe[0].horaExtra) * 100}%` }}
                    />
                  </div>
                </div>
              </div>
