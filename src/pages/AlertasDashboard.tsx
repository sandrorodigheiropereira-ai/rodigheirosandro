...
function AlertRow({ alert, index }: { alert: Alert; index: number }) {
  ...
  return (
    ...
    >
      <Icon
        className={`w-4 h-4 mt-0.5 shrink-0 text-${alert.type === "danger" ? "danger" : alert.type === "warning" ? "warning" : "info"}`}
      />
      <div className="min-w-0 flex-1">
        ...
        {alert.regional && <p className="text-[10px] text-muted-foreground/60 mt-0.5">{alert.regional}</p>}
      </div>
    </motion.div>
  );
}
...
export default function AlertasDashboard() {
  ...
  const allAlerts = useMemo(() => generateAlerts(filtered, sysConfig), [filtered]);
  ...
  const historico = useMemo(() => {
    ...
    const unidadesMap: Record<
      string,
      { unidade: string; regional: string; meses: Record<string, number>; total: number }
    > = {};
    for (const mes of ultimos3) {
      const recsMes = allRecords.filter((r) => r.data === mes && (regional === "all" || r.regional === regional));
      const alertsMes = generateAlerts(recsMes, sysConfig).filter((a) => a.type === "danger");
    ...
    const tipoMap: Record<
      string,
      { categoria: string; tipo: Alert["type"]; meses: Record<string, number>; total: number }
    > = {};
    for (const mes of ultimos3) {
      const recsMes = allRecords.filter((r) => r.data === mes && (regional === "all" || r.regional === regional));
      const alertsMes = generateAlerts(recsMes, sysConfig);
      for (const alert of alertsMes) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          ...
          >
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Ranking de risco</h3>
                ...
                {rankingRisco.map((item, i) => (
                  <div key={item.unidade}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span
                        className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${TIPO_BADGE_CLASS[item.maxTipo]}`}
                      >
          ...
          <motion.div
            ...
            className="glass-card rounded-xl p-5 space-y-3"
          >
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Alertas por tipo</h3>
                ...
                {contadorPorTipo.map(([cat, count]) => {
                  ...
                  return (
                    <div key={cat} className="flex items-center justify-between gap-2">
                      ...
                      <span
                        className={`text-[10px] font-semibold px-1.5 py-0.5 rounded shrink-0 ${TIPO_BADGE_CLASS[tipo]}`}
                      >
                        {count}
                      </span>
                    </div>
        ...
        {saudeRegional.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {saudeRegional.map((r, i) => (
              ...
              >
                <p
                  className={`text-xs font-semibold uppercase tracking-wider ${regionalTextColor(r.danger, r.warning)}`}
                >
                  {r.regional}
                </p>
                ...
                <div className="mt-2 space-y-0.5">
                  {r.danger > 0 && (
                    <p className="text-[10px] text-danger">
                      {r.danger} crítico{r.danger > 1 ? "s" : ""}
                    </p>
                  )}
                  {r.warning > 0 && <p className="text-[10px] text-warning">{r.warning} atenção</p>}
                  {r.info > 0 && <p className="text-[10px] text-info">{r.info} info</p>}
                </div>
              </motion.div>
            ))}
                  ...
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      {(historico.meses || []).map((m) => (
                        <th key={m} className="text-center p-2 pb-3 font-semibold text-muted-foreground w-16">
                          {m}
                        </th>
                      ))}
                    ...
                    {(historico.porUnidade || []).map((u, i) => (
                      <tr key={u.unidade} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                        ...
                        <td className="p-2 py-2.5 text-center">
                          <span
                            className={`text-sm font-bold ${u.total >= 6 ? "text-danger" : u.total >= 3 ? "text-warning" : "text-muted-foreground"}`}
                          >
                            {u.total}
                          </span>
                        </td>
                    ...
                    <div key={t.categoria}>
                      <div className="flex items-center justify-between mb-1 gap-2">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <span
                            className={`text-xs font-bold w-8 text-right ${TIPO_BADGE_CLASS[t.tipo].split(" ")[1]}`}
                          >
                            {t.total}
                          </span>
                        </div>
